...
def update_enemybullet(setting, screen, stats, sb, ship, bulletss, aliens):
    bulletss.update()
    for bullet in bulletss.copy():
        if bullet.rect.top >= setting.height:
            bulletss.remove(bullet)
    check_bullet_ship_collisions(setting, screen, stats, sb, ship, aliens, bulletss)
...
def check_bullet_ship_collisions(setting, screen, stats, sb, ship, aliens, bulletss):
    for bullet in bulletss:
        collision = ship.rect.colliderect(bullet.rect)
        if collision:
            ship_hit(setting, stats, screen, ship, aliens, bulletss)
            ship.center_ship()
            return True
    # print("masuk")
...
def update_screen(screen, setting, sb, ship, stats, bullets, aliens, play_button,bulletss):
